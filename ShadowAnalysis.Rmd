...
## Analysis
Using Equation \ref{eq:phi}, Equation \ref{eq:omega} can be substituted in for \(\phi\). Giving Equation \ref{eq:len}. The length of the ladder at the point shown on the reference point of the Length plot is equivalent to the distance of the International Space Station from the surface of the Earth or 254 \(miles\) (408,773 \(m\)). 
...
ggplot(plot.dat, 
       aes(x = xval, y = values))+ geom_line(color="cyan")+ ggtitle("Instantaneous Velocity Plot")+ xlab("Time(s)")+ ylab("Velocity(m/s)")+ geom_point(shape=1, aes(x=18954.04, y=11000), colour="black", size=2)
...
## Conclusion
Ultimately, the question "How fast would a person have to climb a ladder to constantly see the Sun just about to set" can be answered using a given time (\(t\)) in seconds after which the Sun has set and the Equation \ref{eq:vel}. At an hour after sunset a person would have to be on top of a ladder 221,193 \(m\)'s high and climbing at 126.4 \(\frac{m}{s}\). Which is about 137 \(miles\) and 280 \(mph\).
